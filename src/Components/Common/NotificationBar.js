...
export default function NotificationBar() {
  ...
  const getNotifications = () => {
    ...
    getNotificationsFromIndexedDB()
      ...
      .catch((error) => {
        console.error(
          "Failed to retrieve notifications from IndexedDB:",
          error
        );
      });
  };
  ...
  const handleDeleteNotification = (id) => {
    deleteNotificationFromIndexedDB(id)
      .then(() => {
  ...
  return (
    <div>
      ...
      <div className="  w-full h-[85vh] overflow-y-scroll  ">
        {notiarray?.map((i) => (
          <div
            className="flex w-[90%] flex-row border my-4 items-center  justify-center   h-auto p-2 relative "
            id="children"
            key={i.messageId}
          >
            <button
              className="p-1 m-1 opacity-70 absolute top-0 right-0"
              onClick={() => handleDeleteNotification(i.messageId)}
            >
            ...
            <div className="w-[30%] flex  items-center  justify-center  ">
              <img
                src={logo}
                alt="notication"
                className="w-[2.8rem] h-[3rem]"
              />
            </div>
            <div className="w-[70%] flex  items-center  justify-center  flex-col">
              <h1 className="font-bold ">{i?.title}</h1>
              <p className="text-gray-500">{i?.body}</p>
            </div>
          </div>
        ))}
      </div>
      <p className="text-purple-500" onClick={() => setNotiarray([])}>
