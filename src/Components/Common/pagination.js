  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-center p-4">
      ...
      <span className="text-gray-600">
        {t("Results")}: {(page - 1) * limit + 1} -{" "}
        {Math.min(page * limit, totalRecords)} {t("of")} {totalRecords}
      </span>
        ...
        {getPageNumbers().map((p, index) => (
          <button
            ...
            className={`px-3 py-1 border rounded-md transition-all font-semibold ${
              page === p
                ? "text-white bg-gradient-to-r from-red-400 via-pink-600 via-pink-600 to-purple-500 shadow-md"
