...
const EventCard = ({ event }) => {
  // Define color classes based on the event type or other properties
  const bgColors = ['bg-pink-500', 'bg-purple-500', 'bg-blue-500', 'bg-indigo-500'];
  const bgColor = bgColors[event.id % bgColors.length]; // Cycle through colors based on id
    ...
    <div className="h-full flex-1 border  flex-wrap   rounded-lg shadow-lg text-white bg-[#FF6C9C] border-black" >
        <div className=" flex  items-center justify-around  p-4 gap-2 ">
          <div className=" border  rounded  p-2 bg-[#F9FAFC]">
            <span className="text-3xl font-semibold text-[#FF6C9C]">  {event.startDate.getDate()}</span>
          </div>
          ...
          <div className="flex  flex-col flex-wrap justify-start  " >
            ...
            <FontAwesomeIcon  style={{color:"white", background:' ' ,margin:'0',padding:'0',  height:'20px' ,width:'20px' ,borderRadius:'50%',marginRight:'5px' }} icon={faClock} />
                          ...
                          <span className='font-xs  font-[350]'>{`${event.startDate.getHours()}:${String(event.startDate.getMinutes()).padStart(2, '0')} AM`}</span>
