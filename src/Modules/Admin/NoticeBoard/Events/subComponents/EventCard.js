...
const EventCard = ({ event }) => {
  ...
  const rawTime = event?.time || t("No time"); // Default fallback for missing time
    ...
    >
      ...
      <div className="flex flex-col mt-2  text-sm font-semibold leading-5">
        <p className="truncate">
          {truncateText(event?.description, 20) || t("No description available")}
        </p>
        <span>{event?.date ? new Date(event.date).toLocaleDateString() : t("Invalid date")}</span>
      </div>
    </div>
  );
};
