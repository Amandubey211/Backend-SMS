...
import { setEditFormData, setFilters, setModalOpen, setSidebarEditOpen, setSidebarOpen } from "../../../../Store/Slices/Admin/Accounting/StudentFees/studentFeesSlice";
import { deleteStudentFee, fetchClasses, fetchFees } from "../../../../Store/Slices/Admin/Accounting/StudentFees/studentFees.action";
const AccountingSection = () => {

  const { isSidebarOpen, isSidebarEditOpen, feesData, filters, editFormData, isModalOpen } = useSelector((store) => store?.admin?.student_fees)
  ...
  const filteredData = feesData.filter((item) => {
    return (
      (filters.class === "" || item?.studentId?.presentClassId?.className === filters.class) &&
            ...
            <div className="flex items-center space-x-4">
              {["Everyone", "Paid", "Unpaid"].map((status) => (
                <label key={status} className="flex items-center cursor-pointer">
                  ...
                  <span
                    className={`transition-colors duration-200 ${filters.status === status ? "text-red-700" : "text-gray-700"
                      }`}
                  >
                    {status}
                  </span>
