...
const TeacherRow = React.memo(({ teacher, onPayClick }) => (
  <tr className="bg-white">
    ...
    <td className="px-5 py-2 border-b border-gray-200">{teacher.schoolId}</td>
    ...
    <td className="px-5 py-2 border-b border-gray-200">
      {teacher.paidDate ? new Date(teacher.paidDate).toLocaleDateString() : "Not Paid"}
    </td>
    <td className="px-5 py-2 border-b border-gray-200">
      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${teacher.status === "Paid" ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}`}>
        {teacher.status}
      </span>
    </td>
    <td className="px-5 py-2 border-b border-gray-200">
      ...
      <table className="min-w-full leading-normal mt-4 shadow-lg rounded-lg overflow-hidden">
        <thead>
          <tr className="text-left text-gray-700 bg-gray-100">
            <th className="px-5 py-3 border-b-2 border-gray-200">Teacher's ID</th>
            <th className="px-5 py-3 border-b-2 border-gray-200">School ID</th>
