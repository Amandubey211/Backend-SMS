...
import { updateFinancialYear } from "../../../Store/Slices/Common/FinancialYear/financialYear.action";
...
const EditFinanceYearModal = ({ show, onClose, year, FinanceYears }) => {
  const { t } = useTranslation("admFinanceYear");
  const dispatch = useDispatch();
  const isSingle = FinanceYears?.length === 1;
  ...
  useEffect(() => {
    if (year && show) {
      setFormData({
        ...
        // Force active status if only one Finance year exists
  ...
  const validateForm = () => {
    let formErrors = {};
    if (!formData.Name) {
      formErrors.Name = t("Finance Year is required.");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(updateFinancialYear({ data: formData, id: year?._id }));
  };
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-3 flex items-center">
              <input
                ...
                disabled={isSingle} // Disable if only one Finance year exists
...
export default EditFinanceYearModal;
