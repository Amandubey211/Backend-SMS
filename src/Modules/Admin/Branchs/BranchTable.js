...
import { fetchBranch, updateBranch } from "../../../Store/Slices/Admin/branchs/branch.action";
...
const BranchTable = () => {
  ...
  const {branchs,loading,error} = useSelector((store)=>store.common.branchs)
      useEffect(()=>{
        if(!selectBranch){
      ...
      const changeBranch = (branchId)=>{
        ...
        dispatch(updateBranch({navigate,data})).then(()=>{setLocalCookies('SelectedschoolId',branchId);window.location.reload()});
      }
  return (
    <div className="bg-white p-2 rounded-lg w-full px-8">
       <h2 className="text-2xl text-gray-600 flex mb-2 flex-row items-center gap-2"> <span><LuSchool/></span>{userDetails?.schoolName}</h2>
      {loading?<Spinner/>:
        ...
        <tbody className="text-base text-gray-700">
          {branchs?.map((b) => (
            <tr
              key={b._id}
              className={`${
                selectBranch == b._id ? "bg-green-50 hover:bg-green-100" : "hover:bg-gray-50"
