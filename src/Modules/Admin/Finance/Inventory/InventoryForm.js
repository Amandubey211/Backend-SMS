...
const handleSubmit = async (values) => {
  ...
  const payload = {
    ...
    supplierId: selectedEntity?._id, // Ensure supplierId is included
  };

  if (editData) {
    await dispatch(updateInventory({ id: editData._id, ...payload })).then((res) => {
      if (res?.payload?.success) {
        onClose();
      }
    });
  } else {
    await dispatch(createInventory(payload)).then((res) => {
      if (res?.payload?.success) {
        onClose();
      }
    });
