...
const StudentGradePieChart = () => {
  ...
  const labels =
    studentSubjectProgress?.map((subject) => subject.subjectName) || [];
  const dataValues = studentSubjectProgress?.map(
    (subject) => subject.percentageValue
  );
    ...
    datasets: [
      {
        ...
        backgroundColor: allZero
          ...
          : dataValues?.map((value) =>
              value === 0
                ? "gray"
                : colors[dataValues.indexOf(value) % colors?.length]
            ),
