...
import { activeHelper, deactiveHelper } from "../../../../Store/Slices/Transportation/Helper/helper.action";

const ProfileHelperCard = ({ profile, onClick, editUser }) => {
  const dispatch = useDispatch();
  const { loading } = useSelector((store) => store.transportation.transportHelper);
  ...
  const deleteTeacher = () => {
    ...
    dispatch(deactiveHelper(userData));
  ...
  const activateUser = (e, id, role) => {
    e.stopPropagation();
    const userData = { id: id, role, active: true };
    dispatch(activeHelper(userData));
  };
...
export default ProfileHelperCard;
