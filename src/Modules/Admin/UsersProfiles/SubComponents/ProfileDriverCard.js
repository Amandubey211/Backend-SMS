...
import { activeDriver, deactiveDriver } from "../../../../Store/Slices/Transportation/Driver/driver.action";

const ProfileDriverCard = ({ profile, onClick, editUser }) => {
  const dispatch = useDispatch();
  const { loading } = useSelector((store) => store.transportation.transportDriver);
  ...
  const deleteTeacher = () => {
    ...
    dispatch(deactiveDriver(userData));
  ...
  const activateUser = (e, id, role) => {
    ...
    dispatch(activeDriver(userData));
  };
...
export default ProfileDriverCard;
