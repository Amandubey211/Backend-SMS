...
const AddDriver = ({ role, data }) => {
    ...
    if (data) {
      ...
      setTeacherData({
        ...
        resignationDate: data.resignationDate
          ? new Date(data.resignationDate).toISOString().split("T")[0]
          : "",
  ...
  const validateForm = () => {
    ...
    if (!/^\+?[0-9\s-()]{7,15}$/.test(contactNumber)) {
      toast.error(
        "Please enter a valid contact number (7-15 digits, can include +, spaces, -, ())."
      );
      return false;
    }
    ...
    if (!/^[a-zA-Z0-9-]{5,20}$/.test(national_Id)) {
      toast.error(
        "Please enter a valid National ID (5-20 alphanumeric characters or hyphens)."
      );
      return false;
    }
      ...
      try {
        ...
        if (addResult && addResult.success) {
          ...
          if (selectedRoleIds.length > 0 && addResult._id) {
            ...
            toast.warn(
              "Driver added, but could not assign roles due to missing ID from add response."
            );
          }
        } else {
        ...
        toast.error(
          err?.message || "An error occurred while adding the driver."
        );
      } finally {
        setIsSubmitting(false);
      }
    ...
    >
      <form className="space-y-8" onSubmit={handleSubmit} noValidate>
        ...
        >
          ...
          <h2 className="text-xl font-semibold border-b py-4">
              ...
              <div className="transform scale-80 origin-left mt-1">
                <Switch
                  ...
                  onChange={(checked) =>
                    setTeacherData((prev) => ({
                      ...prev,
                      policeVerificationDone: checked,
                    }))
                  }
                />
              </div>
                  ...
                  ? [
                      {
                        uid: "-1",
                        name:
                          typeof teacherData.teacherCV === "string"
                            ? teacherData.teacherCV.split("/").pop()
                            : teacherData.teacherCV.name,
                        status: "done",
                        originFileObj:
                          teacherData.teacherCV instanceof File
                            ? teacherData.teacherCV
                            : undefined,
                        url:
                          typeof teacherData.teacherCV === "string"
          ...
          >
            <ProtectedSection requiredPermission={PERMISSIONS.ASSIGN_ROLE}>
              ...
              <Select
                ...
                notFoundContent={
                  filteredRoleItems.length === 0 && allRoleItems.length > 0
