...
//           "http://localhost:8080/student/my_fees",
...
const FinanceTable = () => {
  ...
  const [feesDetails, setFeesDetails] = useState([]);
  const [totalUnpaidFees, setTotalUnpaidFees] = useState("0 QR");
  const [totalPaidFees, setTotalPaidFees] = useState("0 QR");
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchFeesDetails = async () => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:8080/student/my_fees', {
          headers: {
            'Authentication': token
          }
        });
