...
function StudentReportCard() {
    ...
    // console.log("StudentDetails", userDetails);
    ...
    const fetchPdf = async () => {
        ...
        setLoading(true);
        try {
            const response = await dispatch(fetchStudentReportCard(userDetails.userId));
            if (response.error) {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            saveAs(blob, `${userDetails.firstName}_${userDetails.lastName}_ReportCard_FirstPage.pdf`);
        } catch (err) {
