  ...
  useEffect(() => {
    const fetchPageData = async () => {
      try {
        ...
        toast.error("Failed to fetch page data: " + error.message);
      }
    };
  ...
  return (
    <div className="flex">
      ...
      <div className="border-l">
        <Header />
        <div className="p-6 bg-white">
          <h1 className="text-3xl font-semibold mb-4">{pageData.title}</h1>
          <p className="text-gray-700 mb-6">{pageData.content}</p>
          <div className="relative"></div>
        </div>
      </div>
    </div>
